...
diff_df_max_sort_df = pd.DataFrame(tog_max, columns=['col_event', 'row_event', 'Change in probability']) 
diff_df_max_sort_df.insert(2, "Events", [str(m)+'-'+str(n) for m,n in zip(row_max, col_max)]) 
...
diff_df_min_sort_df = pd.DataFrame(tog_min, columns=['col_event', 'row_event', 'Change in probability']) 
diff_df_min_sort_df.insert(2, "Events", [str(m)+'-'+str(n) for m,n in zip(row_min, col_min)]) 
...
ax = sns.barplot(x="Events", y="Change in probability", data = diff_df_max_sort_df, palette=np.array(max_pal[::1]), order=diff_df_max_sort_df.sort_values('Change in probability').Events, ax=axes[0]).set_title("Positive probability change")
ax = sns.barplot(x="Events", y="Change in probability", data = diff_df_min_sort_df, palette=np.array(min_pal[::1]), order=diff_df_min_sort_df.sort_values('Change in probability').Events, ax=axes[1]).set_title("Negative probability change")
for ax in axes:
